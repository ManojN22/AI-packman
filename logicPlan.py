...
def sentence3() -> Expr:
    ...
    res_exp = logic.conjoin(exp_1,exp_2,exp_3)
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    result = conjoin(premise,~conclusion)
    return findModel(result)==False
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for lit in literals:
        for lit2 in literals:
            if lit != lit2:
                conj.append(logic.disjoin(~lit, ~lit2))
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    for lit in literals:
        ...
        for lit1 in literals:
            if (flg):
                disj = logic.disjoin(~lit, ~lit1)
                conj.append(disj)
            if lit == lit1:
                flg = True 
    ans = logic.disjoin(ansList)
    conj.append(ans)
    return logic.conjoin(conj)
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        KB.append(exactlyOne([PropSymbolExpr('North', time=t), PropSymbolExpr('South',  time=t), PropSymbolExpr('East',  time=t), PropSymbolExpr('West',  time=t)]))   
        for (x,y) in non_wall_coords:
